/*
 * Performs WOLA FFT to channelize data. (Single precision version)
 *
...
struct thread_data{
	...
	mxComplexSingle *thread_out; // for R2018
};
...
unsigned __stdcall threaded_wola(void *pArgs){
	...
	mxComplexSingle *out = inner_data->thread_out; // for R2018
	// end of assignments
	...
	for (nprime = t_ID; nprime<nprimePts; nprime=nprime+NUM_THREADS){
        ...
        // memcpy(&out[nprime*N],fout,sizeof(mxComplexSingle)*N); // if you write directly, you won't need to copy it
	}
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
    ...
    f_tap = mxGetSingles(prhs[1]); // no more using GetPr or GetPi
...
//     printf("y[0].re = %g, y[0].im = %g \n",y[0].real,y[0].imag);
    ...
    plhs[0] = mxCreateNumericMatrix(fftlen,nprimePts,mxSINGLE_CLASS,mxCOMPLEX);
